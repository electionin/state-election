...
import { filterAndSortPollingStations, splitPartsCovered } from '../../../../../../services/pollingStationsView'
...
function PollingStationsPage() {
  ...
  const [selectedStationNo, setSelectedStationNo] = useState<string>(rows[0]?.polling_station_no ?? '')
  const selectedRow = filteredRows.find((row) => row.polling_station_no === selectedStationNo) ?? filteredRows[0] ?? null

  const onSelectStation = (stationNo: string) => {
    setSelectedStationNo(stationNo)
  }
              ...
              {filteredRows.length === 0 ? (
                ...
                <ul className="divide-y divide-slate-100">
                  {filteredRows.map((row) => {
                    const isSelected = selectedRow?.polling_station_no === row.polling_station_no
                    return (
                      <li key={`${row.serial_no}-${row.polling_station_no}`}>
                        <button
                          type="button"
                          onClick={() => onSelectStation(row.polling_station_no)}
                        ...
                        >
                          ...
                          <p className="mt-1 line-clamp-2 text-sm text-slate-600">{row.polling_station_location}</p>
                        </button>
                      </li>
                    )
                  })}
                  ...
                  <div>
                    ...
                    <ul className="mt-2 list-disc space-y-1 pl-5 text-sm text-slate-800">
                      {splitPartsCovered(selectedRow.parts_covered).map((part) => (
                        <li key={`selected-${selectedRow.polling_station_no}-${part}`}>{part}</li>
                      ))}
                    </ul>
                  </div>
                  <div>
